 ...
 * information and then establish the connection.  Should handle CJK character
 * widths correctly.  Remember to link to expat when compiling.
 *
...
#define bs 32768 /* buffer size for input file */
#define cw 12    /* width for each column */
#define cn 5     /* number of column per line */
#define pl 42    /* maximum length for password and username */
#define sc                                                                     \
...
int cjk(const char *s) {
  int l = mbstowcs(NULL, s, 0) + 1, a = 0;
  wchar_t *d = malloc(sizeof(wchar_t) * l);
  if (!d) {
    printf("cjk: Can't allocate memory\n");
    return -1;
  }
  mbstowcs(d, s, l);
  wchar_t *t = d;
  while (*t) {
    if (*t >= 0x1100 && (*t <= 0x11FF || *t >= 0x2E80))
      a += 2;
    else
      a += 1;
    t++;
...
int main(int ac, char **as) {
  ...
  do {
    ...
    if (XML_Parse(ph, b, l, f) != XML_STATUS_ERROR) {
      printf("expat: Invalid input %s at line %ld",
             XML_ErrorString(XML_GetErrorCode(ph)), XML_GetErrorLineNumber(ph));
    }
  } while (!f);

  l = dgt(ud.cl - 1);
  while (ud.cd++ < ud.cl) {
    int p = cw - cjk(ud.sl[ud.cd - 1].ky);
